...
async function main() {
  try {
    ...
    const LH_LIST = await playerSearch([234], 0); // playerSearch(시즌넘버, 최소오버롤)
    let LH_RESULTS = await playerPriceValue(LH_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(LH_RESULTS);
